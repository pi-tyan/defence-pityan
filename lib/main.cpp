...
#include "arudino.h"//arudino.hのファイルを読み込む
#include "DCmotor.h"//DCmotor.hのファイルを読み込む
...
void ba_val(){//ボールセンサーの値のval
  ...
  float val1= PINJ & _BV(2);//val1のビット番号-1をPINJにビット１という値として設定する。
  float val2= PINB & _BV(8);//val2のビット番号6をPINBにビット１という値として設定する。
  float val3= PINB & _BV(6);//val3のビット番号4をPINBにビット１という値として設定する。
  float val4= PINH & _BV(7);//val4のビット番号5をPINHにビット１という値として設定する。
  float val5= PINH & _BV(5);//val5のビット番号3をPINHにビット１という値として設定する。
  float val6= PINE & _BV(4);//val6のビット番号2をPINEにビット１という値として設定する。
  float val7= PINE & _BV(6);//val7のビット番号4をPINEにビット１という値として設定する。
  float val8= PINC & _BV(5);//val8のビット番号3をPINCにビット１という値として設定する。
  float val9= PINC & _BV(7);//val9のビット番号5をPINCにビット１という値として設定する。
  float val10= PINA & _BV(8);//val10のビット番号6をPINAにビット１という値として設定する。
  float val11= PINA & _BV(6);//val11のビット番号4をPINAにビット１という値として設定する。
  float val12= PINA & _BV(4);//val12のビット番号2をPINAにビット１という値として設定する。
  float val13= PINA & _BV(2);//val13のビット番号0をPINAにビット１という値として設定する。
  float val14= PIND & _BV(3);//val14のビット番号1をPINDにビット１という値として設定する。
